...
const volumeCard = L => {
  if (!L.length) return '';
  const buckets = {};
  L.forEach(l => { const wk = monday(dOf(l.workout_sessions.started_at)), cat = l.workout_exercises.exercises.category || '', v = +l.weight * l.reps; buckets[wk] ??= { total: 0, cats: {} }; buckets[wk].total += v; buckets[wk].cats[cat] = (buckets[wk].cats[cat] || 0) + v; });
  const curW = monday(today()), prevW = monday(new Date(new Date(curW + 'T12:00:00').getTime() - 7 * 864e5).toLocaleDateString('sv-SE'));
  const cur = buckets[curW] || { total: 0, cats: {} }, prev = buckets[prevW] || { total: 0, cats: {} };
  const delta = prev.total ? Math.round((cur.total - prev.total) / prev.total * 100) : null;
  if (!cur.total && !prev.total) return '';
  return `<div class="card"><b>📦 Volumen esta semana</b><br><span class="big">${Math.round(cur.total).toLocaleString('es-ES')} kg</span> ${delta !== null ? `<span class="tag ${delta < 0 ? 'neg' : ''}">${delta > 0 ? '+' : ''}${delta}% vs. semana anterior</span>` : ''}
   ${Object.entries(cur.cats).sort((a, b) => b[1] - a[1]).map(([k, v]) => `<div class="volbar ${cc(k)}"><i style="width:${Math.round(v / cur.total * 100)}%"></i><span>${CATS[k] ? CATS[k][0] : 'Otros'}</span><b>${Math.round(v).toLocaleString('es-ES')} kg</b></div>`).join('')}</div>`;
};
...
const TUTORIAL = `<details class="card"><summary><b>📖 Tutorial</b></summary>
 ...
 <details class="tut"><summary>🕘 Historial</summary><div>Todas las sesiones que has completado, agrupadas por semana. El círculo de color junto a la fecha es tu RPE de esa sesión (verde, esfuerzo bajo; rojo, esfuerzo máximo). Toca una para ver los pesos y repeticiones que apuntaste (y editarla si han pasado menos de 3 días).</div></details>
 <details class="tut"><summary>🏋️ Ejercicios</summary><div>Cada ejercicio que has hecho, con tu último peso, tu mejor peso y la diferencia desde la primera vez. Tócalo para ver la gráfica de tu progresión y el historial. (Los ejercicios por tiempo, como planchas o sprints, no aparecen aquí todavía.)</div></details>
...
const T = {
  ...
  async c(h) {
    ...
    html(`<a href="#/">← Clientes</a><div class="hero pf"><div class="av">${esc(ini)}</div><div><h2>${tagBadge(p)}${esc(p.nombre + ' ' + p.apellidos)}</h2><small>${esc(p.email)}</small></div></div>
     ...
     <div class="card"><b>🏷️ Identificar a este cliente</b><p class="muted">Solo lo ves tú, te ayuda a reconocerlo rápido en tus listas.</p>
     ...
     <div class="emo">${EMOJIS.map(em => `<button class="emob ${p.client_emoji === em ? 'on' : ''}" data-tag="client_emoji" data-v="${em}">${em}</button>`).join('')}</div></div>
  ...
  async seg() {
    const s = await ok(sb.from('workout_sessions').select('id,completed_at,rpe,observations,profiles(nombre,apellidos,client_color,client_emoji),workout_days(day_name,title)').eq('completed', true).order('completed_at', { ascending: false }).limit(60));
    html(`<h2>Seguimiento</h2>${s.map(x => `<a class="card link" href="#/ver/${x.id}">${tagBadge(x.profiles)}<b>${esc(x.profiles.nombre + ' ' + x.profiles.apellidos)}</b> · ${fd(x.completed_at)}<br>${esc(x.workout_days.day_name + ' — ' + x.workout_days.title)} · RPE ${x.rpe}/10${x.observations ? `<p class="quote">“${esc(x.observations)}”</p>` : ''}</a>`).join('') || '<p class="muted">Aún no hay sesiones completadas.</p>'}`);
  },
  async ej(h) {
