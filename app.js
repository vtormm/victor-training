...
const stat = (ws, ses) => {
  const w = curWeek(ws); if (!w) return {};
...
const T = {
  ...
  async c(h) {
    ...
    const st = stat(ws, ses), rp = ses.slice(0, 3).map(s => s.rpe).filter(Boolean);
    html(`<a href="#/">← Clientes</a><h2>${esc(p.nombre + ' ' + p.apellidos)}</h2><div class="card">${esc(p.email)}<br>
     ${st.w ? `Semana ${st.w.week_number} · ${st.done}/${st.days.length} sesiones<br>Próxima: ${st.next ? esc(st.next.day_name + ' · ' + st.next.title) : '—'}<br>` : 'Sin programación<br>'}
     Última: ${ses[0] ? esc(fd(ses[0].completed_at) + ' · ' + ses[0].workout_days.title) : '—'}<br>RPE reciente: ${rp.length ? (rp.reduce((a, b) => a + b, 0) / rp.length).toFixed(1) + '/10' : '—'}
     ${ses.filter(s => s.observations).slice(0, 2).map(s => `<p class="quote">“${esc(s.observations)}”</p>`).join('')}</div>
     ...
     ${ws.map(w => `<a class="card link" href="#/w/${w.id}">Semana ${w.week_number} <small class="muted">· desde ${fd(w.week_start + 'T12:00:00')} · ${w.workout_days.length} días</small></a>`).join('') || '<p class="muted">Crea la primera semana.</p>'}
     <div class="row"><h3>Seguimiento</h3><a href="#/c/${id}/ej">Historial de ejercicios →</a></div>
     ${ses.map(s => `<a class="card link" href="#/ver/${s.id}">${fd(s.completed_at)} · ${esc(s.workout_days.day_name + ' — ' + s.workout_days.title)}<br><small>RPE ${s.rpe}/10${s.observations ? ' · ' + esc(s.observations.slice(0, 80)) : ''}</small></a>`).join('') || '<p class="muted">Sin sesiones completadas.</p>'}`);
    $app.onclick = async e => {
  ...
  async w(h) {
    const id = h[1];
    const [w, lib] = await Promise.all([
      ok(sb.from('workout_weeks').select('*,profiles(nombre,apellidos),workout_days(*,workout_exercises(*,exercises(name)))').eq('id', id).single()),
      ok(sb.from('exercises').select('id,name').order('name'))]);
    ...
    html(`<a href="#/c/${w.client_id}">← ${esc(w.profiles.nombre)}</a><div class="row"><h2>Semana ${w.week_number}</h2><button data-a="addday">+ Día</button></div>
     ${days.map(d => `<div class="card"><div class="row"><b>${esc(d.day_name)} — ${esc(d.title)}</b><button class="ghost sm" data-a="delday" data-id="${d.id}">Eliminar día</button></div>
      ${d.workout_exercises.map((x, i) => `<div class="ex"><div><b>${i + 1}. ${esc(x.exercises.name)}</b><br><small>${x.sets} × ${x.reps_min}–${x.reps_max} · RIR ${x.rir_min}–${x.rir_max}${x.trainer_notes ? ' · ' + esc(x.trainer_notes) : ''}</small></div>
    ...
    function dayForm() {
      const m = modal(`<form id="df"><h3>Nuevo día</h3><select name="n">${['Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo'].map(x => `<option>${x}</option>`).join('')}</select><input name="t" placeholder="Título (ej. Pierna + empuje)" required maxlength="80"><button>Crear</button></form>`);
      m.querySelector('#df').onsubmit = async ev => { ev.preventDefault(); const f = new FormData(ev.target); try { await ok(sb.from('workout_days').insert({ week_id: id, day_name: f.get('n'), title: f.get('t').trim(), day_order: days.length + 1 })); m.remove(); route(); } catch (er) { console.error(er); toast('No se ha podido crear el día.'); } };
    }
    function exForm(d, x) {
      ...
      const m = modal(`<form id="xf"><h3>${x ? 'Editar' : 'Añadir'} ejercicio</h3><select name="ex">${lib.map(e => `<option value="${e.id}" ${x?.exercise_id === e.id ? 'selected' : ''}>${esc(e.name)}</option>`).join('')}</select>
  ...
  async ej() {
    const L = await ok(sb.from('exercises').select('*').order('name'));
    html(`<h2>Biblioteca de ejercicios</h2><form id="ef" class="card"><input name="n" placeholder="Nombre" required maxlength="80"><input name="g" placeholder="Grupo muscular" maxlength="40"><textarea name="d" placeholder="Descripción / instrucciones" maxlength="1000"></textarea><input name="v" type="url" placeholder="URL de vídeo (opcional)"><button>Crear ejercicio</button></form>
     ${L.length ? '' : '<button id="sd">Cargar ejercicios básicos</button>'}
     ${L.map(x => `<div class="card row"><div><b>${esc(x.name)}</b> <small class="muted">${esc(x.muscle_group || '')}</small>${x.video_url ? ` <a href="${esc(x.video_url)}" target="_blank" rel="noopener">vídeo</a>` : ''}${x.description ? `<br><small>${esc(x.description)}</small>` : ''}</div><button class="sm ghost" data-id="${x.id}">✕</button></div>`).join('')}`);
    document.getElementById('ef').onsubmit = async ev => { ev.preventDefault(); const f = new FormData(ev.target); try { await ok(sb.from('exercises').insert({ trainer_id: me.id, name: f.get('n').trim(), muscle_group: f.get('g').trim() || null, description: f.get('d').trim() || null, video_url: f.get('v').trim() || null })); route(); } catch (e) { console.error(e); toast('No se ha podido crear (¿nombre repetido?).'); } };
    ...
    $app.onclick = async e => { const id = e.target.dataset.id; if (!id || !confirm('¿Eliminar ejercicio?')) return; try { await ok(sb.from('exercises').delete().eq('id', id)); route(); } catch (er) { console.error(er); toast('No se puede eliminar: está en uso en alguna programación.'); } };
  }
};
...
const C = {
  async home() {
    if (!me.trainer_id) return html(`<h2>Hola, ${esc(me.nombre)}</h2><div class="card">Aún no tienes entrenador asignado. Abre el enlace de invitación que te ha enviado Víctor.</div>`);
    ...
    const st = stat(ws, ses), sid = Object.fromEntries(ses.map(s => [s.workout_day_id, s.id]));
    html(`<h2>Hola, ${esc(me.nombre)}</h2>${!st.w ? '<div class="card">Tu entrenador aún no ha programado tu semana.</div>' : `
     <h3>Esta semana</h3><div class="big">${st.done} / ${st.days.length} <small>sesiones completadas</small></div>
     ${st.next ? `<div class="card"><small class="muted">Próxima sesión</small><h3>${esc(st.next.day_name)}</h3>${esc(st.next.title)}<br><a class="btn" href="#/sesion/${st.next.id}">EMPEZAR SESIÓN</a></div>` : '<div class="card">¡Semana completada! 💪</div>'}
     ${st.days.map(d => sid[d.id] ? `<a class="card link" href="#/ver/${sid[d.id]}">✓ ${esc(d.day_name)} — ${esc(d.title)}</a>` : `<a class="card link" href="#/sesion/${d.id}">${esc(d.day_name)} — ${esc(d.title)}</a>`).join('')}`}`);
  },
  async hist() {
  ...
  async sesion(h) {
    ...
    if (s?.completed) { location.hash = '#/ver/' + s.id; return; }
    ...
    html(`<a href="#/">← Inicio</a><h2>${esc(d.day_name)} — ${esc(d.title)}</h2><form id="sf">
     ...
     <div class="card"><b>RPE / fatiga</b><br><small class="muted">1 — Muy fácil · 10 — Máximo esfuerzo</small><div class="rpe">${Array.from({ length: 10 }, (_, i) => `<label><input type="radio" name="rpe" value="${i + 1}"><span>${i + 1}</span></label>`).join('')}</div></div>
     <div class="card"><b>Observaciones</b><textarea name="obs" maxlength="2000" placeholder="¿Cómo ha ido?"></textarea></div>
    ...
    document.getElementById('sf').onsubmit = async ev => {
      ...
      try {
        const se = await ok(sb.from('workout_sessions').upsert({ client_id: me.id, workout_day_id: d.id, started_at: s?.started_at || new Date().toISOString(), completed: false }, { onConflict: 'client_id,workout_day_id' }).select().single());
        await ok(sb.from('set_logs').upsert(rows.map(r => ({ ...r, session_id: se.id })), { onConflict: 'session_id,workout_exercise_id,set_number' }));
        await ok(sb.from('workout_sessions').update({ completed: true, completed_at: new Date().toISOString(), rpe, observations: f.get('obs').trim() || null }).eq('id', se.id));
        toast('Sesión guardada'); location.hash = '#/hist';
