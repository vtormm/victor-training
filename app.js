...
const CATS = { superior: ['Tren superior', 'k-sup'], inferior: ['Tren inferior', 'k-inf'], funcional: ['Funcional (sin máquina)', 'k-fun'] };
...
const T = {
  ...
  async c(h) {
    ...
    const chips = st.w ? '<div class="chips">' + st.days.map(d => `<span class="chip ${st.doneSet.has(d.id) ? 'done' : 'pend'}">${esc(d.day_name)}</span>`).join('') + '</div>' : '';
    html(`<a href="#/">← Clientes</a><h2>${esc(p.nombre + ' ' + p.apellidos)}</h2><div class="card">${esc(p.email)}<br>
     ...
     ${chips}<div class="row"><h3>Programación</h3><span><button data-a="new">+ Semana</button> ${ws[0] ? '<button data-a="copy" class="ghost">Copiar última</button>' : ''}</span></div>
  ...
  async ej() {
    ...
    const row = x => `<div class="card row it ${cc(x.category)}"><div><b>${esc(x.name)}</b> <small class="muted">${esc(x.muscle_group || '')}</small>${x.video_url ? ` <a href="${esc(x.video_url)}" target="_blank" rel="noopener">vídeo</a>` : ''}${x.description ? `<br><small>${esc(x.description)}</small>` : ''}</div><button class="sm ghost" data-id="${x.id}">✕</button></div>`;
    html(`<h2>Biblioteca de ejercicios</h2><form id="ef" class="card"><input name="n" placeholder="Nombre" required maxlength="80"><select name="c">${Object.entries(CATS).map(([k, [n]]) => `<option value="${k}">${n}</option>`).join('')}</select><input name="g" list="mg" placeholder="Músculo (ej. Pecho)" maxlength="40"><datalist id="mg">${mg.map(x => `<option value="${x}">`).join('')}</datalist><textarea name="d" placeholder="Descripción / instrucciones" maxlength="1000"></textarea><input name="v" type="url" placeholder="URL de vídeo (opcional)"><button>Crear ejercicio</button></form>
    ...
    document.getElementById('ef').onsubmit = async ev => { ev.preventDefault(); const f = new FormData(ev.target); try { await ok(sb.from('exercises').insert({ trainer_id: me.id, name: f.get('n').trim(), category: f.get('c'), muscle_group: f.get('g').trim() || null, description: f.get('d').trim() || null, video_url: f.get('v').trim() || null })); route(); } catch (e) { console.error(e); toast('No se ha podido crear (¿nombre repetido?).'); } };
...
const C = {
  async home() {
    ...
    html(`<h2>Hola, ${esc(me.nombre)}</h2>${!st.w ? '<div class="card">Tu entrenador aún no ha programado tu semana.</div>' : `
     ...
     ${st.days.map(d => sid[d.id] ? `<a class="card link done" href="#/ver/${sid[d.id]}">✓ ${esc(d.day_name)} — ${esc(d.title)}</a>` : `<a class="card link pend" href="#/sesion/${d.id}">${esc(d.day_name)} — ${esc(d.title)}</a>`).join('')}`}`);
  },
  async hist() {
  ...
  async sesion(h) {
    ...
    html(`<a href="#/">← Inicio</a><h2>${esc(d.day_name)} — ${esc(d.title)}</h2><form id="sf">
     ...
     <div class="card"><b>¿Cómo te has sentido?</b><div class="rpe">${Array.from({ length: 10 }, (_, i) => `<label><input type="radio" name="rpe" value="${i + 1}"><span style="--h:${130 - i * 13}">${RPE[i][0]}<small>${i + 1}</small></span></label>`).join('')}</div><p id="rl" class="rl muted">Toca cómo ha sido de duro</p></div>
