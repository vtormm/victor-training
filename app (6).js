...
async function exView(cid, back) {
  ...
  const g = {};
  L.forEach(l => { const x = l.workout_exercises, k = x.exercise_id, d = l.workout_sessions.started_at, w = +l.weight, rm = w * (1 + l.reps / 30); g[k] ??= { n: x.exercises.name, c: x.exercises.category, by: {}, rm: 0 }; const o = g[k].by[d]; if (!o || w > o.w || (w === o.w && l.reps > o.r)) g[k].by[d] = { w, r: l.reps }; if (rm > g[k].rm) g[k].rm = rm; });
  const chart = pts => {
  ...
  async home() {
    ...
    html(`<h2>Hola, ${esc(me.nombre)}</h2><div class="row"><h3>Mis clientes</h3><button id="inv">+ Invitar</button></div>
     ${cl.map(c => {
       ...
       return `<a class="card link" href="#/c/${c.id}"><div class="row"><b>${esc(c.nombre + ' ' + c.apellidos)}</b>${ad.pct !== null ? `<span class="tag ${ad.pct < 70 ? 'neg' : ''}">${ad.pct}%${ad.streak >= 2 ? ' · 🔥' + ad.streak : ''}</span>` : ''}</div>${st.w ? `Semana ${st.w.week_number} · ${st.done}/${st.days.length} sesiones` : 'Sin programación'}
